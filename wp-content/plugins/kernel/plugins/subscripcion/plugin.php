...
class Subscripcion {
	...
	public function __construct() {
		...
		//-- Admin
		self::$PAGADMIN_TITULO = "Subscripcion";
		self::$PAGADMIN_SECCION = "Subscripcion";
	...
	<?php
		...
		wp_mail($datos['email'], $titulo_pag.' - Subscripcion', $cuerpo_correo, $cabeceras);
	}
	...
	function correoAdmin($datos) {
		ob_start();
	?>
		<!-- Una persona ha utilizado el formulario de subscripcion, aquí está la información recolectada:<br><br> -->
		Someone has been request registered at subscription form, here the information:<br><br>
		<table>
			<tbody>
<!-- 
				<tr>
				...
				<tr>
					<td><span style="font-weight: bold;">Tema interés</span></td>
					<td> : </td>
...
-->
	...
	<?php
		...
		wp_mail('drmartinkf@gmail.com', $titulo_pag.' - Subscripcion', $cuerpo_correo, $cabeceras);
	}
...
//				wp_register_script('tiny_mce', self::$PLUGIN_URL.'/assets/js-admin/form/script.js');
	...
	?>
		...
		?>
		<?php 
			$subscripcion     = new SubscripcionDAO();
			$registros        = $subscripcion->getAll($pag_act, $pag_ppp);
			$registros_pagTot = $subscripcion->getTotalPages($pag_ppp);
	...
	public function vistaADMIN_Settings() {
	?>
		<div class="wrap form-admin">
			<div class="icon32 icon32-posts-autos" id="icon-edit"><br></div>
			<h2 class="cabecera"><?php echo self::$PAGADMIN_TITULO; ?> Settings&nbsp;</h2>
		</div>
	<?php
	}
	...
	public function adminMenu() {
		...
		$page_parent = 
			add_menu_page(
				...
				, 'edit_posts'
				, self::$SLUG
				, array($this, 'vistaADMIN')
				, self::$PLUGIN_URL."assets/img-admin/plugins/subscripcion/favicon.ico"
			);
		...
		$page_child  = 
			add_submenu_page(
				...
				, 'edit_posts'
				, self::$SLUG . 'options'
				, array($this, 'vistaADMIN_Settings')
			);
					...
					<form class="form-horizontal form-form" id="form-form">
						<div class="container">
							...
							<div class="row show-grid">
								<div class="col-md-12">
									Sign up to our emails and receive interesting theraphy updates, special workshops and promotions!
								</div>
							</div>
							<div class="row show-grid">
<!-- 
...
-->
								<div class="col-md-4 col-md-offset-6">
									<div class="form-group">
										<div>
											<input style="width:100%;" id="form-email" class="form-control focused required mask-email" name="form[email]" placeholder="Enter you e-mail here" maxlength="200" type="text" value="" />
										</div>
									</div>
								</div>
								<div class="col-md-2">
									<div class="form-group">
										<div class="controls" style="text-align: left;">
											<button id="form-aceptar" type="submit" class="btn btn-aceptar btn-morado" data-loading-text="Sending...">Subscribe</button>
										</div>
									</div>
								</div>
							</div>
...
//				});
				...
				// Subscripcion
//				$('body').on('click', 'a[href$="#subscripcion"]', function (e) {
//					e.preventDefault();
...
//						'email'
	...
	function registroController() {
		...
		$subscripcion = $_POST['form'];

		//var_dump($subscripcion);

		$res = self::validar($subscripcion);

		if($res['error'] == 0):

			$subscripcionDAO = new SubscripcionDAO();

//			if($subscripcionDAO->existe($subscripcion['email'])):
//				$res['msg'] = 'Ya hay un form con ese correo';
//				$res['err'] = 1;
//			else:
//				$ins = $subscripcionDAO->insertar($subscripcion);
//				$res['msg'] = 'Registro insertado con exito: '.$ins;
//				global $email_html;
//				wp_mail($subscripcion['email'], 'Solicitud form recibida', $email_html);
//			endif;

			$ins = $subscripcionDAO->insertar($subscripcion);

			//self::correoUsuario($subscripcion);
			self::correoAdmin($subscripcion);
	...
	public static function install() {
		...
		if ($wpdb->get_var("show tables like '".$subscripcion->getTableName()."'") != $subscripcion->getTableName()) {

			$sql =
			"CREATE TABLE IF NOT EXISTS `".$subscripcion->getTableName()."`
			(
				`id` int(11) NOT NULL AUTO_INCREMENT,
				`nombre` varchar(500) NULL,
				`apellido` varchar(500) NULL,
				`tema_interes` varchar(500) NULL,
